...
Log one JSON object per request (JSON Lines), from provider usage:
  {"conversation": "c-123", "arm": "baseline" | "treatment", "cached": 0, "written": 0, "uncached": 0,
...
multipliers; pass the provider's current price ratios, not the synthetic runs' assumptions.
  ...
  pilot_analysis.py --assign CONVERSATION_ID [--treatment-share 0.5]   print the conversation's arm
...
def analyze(rows: list[dict], write: float = 1.25, read: float = 0.1, output: float = 5.0,
    ...
    for key in ("input_cost", "total_cost"):
        ratio = mean_cost(treat, key) / mean_cost(base, key)
        report[f"{key}_ratio"] = {"treatment_over_baseline": round(ratio, 4), "ci95": bootstrap(
            base, treat, lambda b, t, k=key: mean_cost(t, k) / mean_cost(b, k), samples, seed)}
...
def from_domain(path: str, baseline: str = "front-tuned", treatment: str = "placed") -> list[dict]:
    ...
    for key, runs in saved["scenarios"].items():
        for i, run in enumerate(runs):
            for arm, name in ((baseline, "baseline"), (treatment, "treatment")):
                for r in run[arm]:
...
def main():
    ...
    if args.assign is not None:
        print(assign(args.assign, args.treatment_share, args.salt))
    elif args.from_domain:
        for row in from_domain(args.from_domain):
